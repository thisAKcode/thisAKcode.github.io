...
The main desktop application of Safe Software is FME Workbench. The inteface consist of, among other, canvas. Here user graphically define a translation called a "workspace" wich can be saved for re-use later (fme workspace files on that regard are similar to script files).
...
A classical simple workspace contains:
...
## PythonCaller
PythonCaller transformer used to perform tasks within FME, which are not as smooth with standard FME transformers.
...
2. Add a PythonCaller Transformer.
...
If you need to process all values of given attribute for a bunch of features then you may need use classes. But it is not the only use case.
